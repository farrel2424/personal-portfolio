...
import contactImg from "../assets/img/programmin.jpg";
import 'animate.css';
import TrackVisibility from 'react-on-screen';
import logo from "../assets/img/logogo.png";
                ...
                <form onSubmit={handleSubmit}>
                  <Row className="g-3">
                    <Col size={12} sm={6} className="px-1">
                      <input type="text" value={formDetails.firstName} placeholder="First Name" onChange={(e) => onFormUpdate('firstName', e.target.value)} className="formControl"/>
                    </Col>
                    <Col size={12} sm={6} className="px-1">
                      <input type="text" value={formDetails.lasttName} placeholder="Last Name" onChange={(e) => onFormUpdate('lastName', e.target.value)} className="formControl"/>
                    </Col>
                    <Col size={12} sm={6} className="px-1">
                      <input type="email" value={formDetails.email} placeholder="Email Address" onChange={(e) => onFormUpdate('email', e.target.value)} />
                    </Col>
                    <Col size={12} sm={6} className="px-1">
                      <input type="tel" value={formDetails.phone} placeholder="Phone No." pattern="[0-9]*" inputMode="numeric"  onChange={(e) => {
      const numericValue = e.target.value.replace(/\D/g, ""); 
      onFormUpdate("phone", numericValue);
    }}
